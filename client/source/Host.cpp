...
namespace multislider
{
    ...
    Host::Host(shared_ptr<RakNet::TCPInterface> connection, shared_ptr<RakNet::SystemAddress> address, const std::string & playerName, const std::string & roomName, uint32_t playersLimit, LobbyCallback* callback)
        : mTcp(connection), mCallback(callback), mServerAddress(address)
    {
        ...
        mCallback->onJoined(NULL, mMyRoom, playerName);
        mIsOpened = true;
    }
    //-------------------------------------------------------
    ...
    void Host::closeRoom()
    {
        ...
        mCallback->onLeft(NULL, mMyRoom, mMyRoom.getHostName(), 0);
        mIsOpened = false;
    }
        ...
        for (;;) {
            ...
            if (isMessageClass(messageClass, frontend::BROADCAST)) {
                ...
                if (mMyRoom.deserialize(messageJson.get<Object>(constants::MESSAGE_KEY_ROOM, Object()))) {
                    mCallback->onBroadcast(NULL, mMyRoom, mMyRoom.getHostName(), message);
                }
            }
                ...
                mCallback->onSessionStart(NULL, mMyRoom, mMyRoom.getHostName(), session);
            }
            ++counter;
        }
        return counter;
